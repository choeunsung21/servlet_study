...
<body>
	...
	<h3>3. 객체 꺼내오기</h3><%@page import="com.gn.vo.Person"%>
	<%
	...
	<ol>
		...
		2. getter 알아서 실행됨
		-->
		<li>EL 방식 : ${person.name } : ${person.age }</li>
	</ol>
	<h2>EL의 연산자</h2>
	<%
	// 1. 숫자
	...
	<h3>1. 산술 연산</h3>
	<p>
		10+3=${num1+num2}<br> 10-3=${num1-num2}<br> 10/3=${num1 div num2}<br> 10%3=${num1 mod num2}<br>
	</p>
	<h3>2. 문자열 연결</h3>
	<p>${str1}:${str2}</p>
	<h3>3. 대소 비교</h3>
	<p>
		num1이 num2보다 큰가요? : ${num1 gt num2}<br> num1이 num2보다 작거나 같은가요? : ${num1 le num2}<br>
	</p>
	<h3>4. 동등 비교</h3>
	<p>
		숫자 일치 : ${num1 eq 10 }<br> 숫자 불일치 : ${num2 ne 3 }<br> 문자 일치 : ${str1 eq str2 }<br> 문자 불일치 : ${str1 ne str2 }<br>
	</p>
	<h3>5. 비어있는지 확인</h3>
	<ol>
	...
	<h3>6. 논리 연산자</h3>
	<p>${true and true }<br> ${num1 eq 10 and num2 eq 3 }<br> ${false or true }
	</p>
	<hr>
	<h1>JSTL</h1>
	<h2>1. 변수</h2>
	...
	<c:choose>
		...
		<c:when test="${num1 ge 10 }">
			<p>num1이 10보다 크거나 같으면서, 20보다도 작거나 같나요?</p>
		</c:when>
		<c:otherwise>
			<p>num1이 10보다 작습니까?</p>
		</c:otherwise>
	...
	<p>
		<c:out value="${titleParam}${boardTitle}"></c:out>
	</p>
	<hr>
	...
	<fmt:formatNumber value="1234.567" type="currency" />
	<br>
	...
	<br>
	<fmt:formatDate value="${parsedDate }" pattern="yy년 MM월" />
	<br>
	...
	<p>
		<c:out value="${fn:contains(data,'You')?'O':'X' }" />
	</p>
