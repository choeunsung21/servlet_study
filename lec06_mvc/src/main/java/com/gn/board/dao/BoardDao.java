...
public class BoardDao {
	...
	public List<Board> selectBoardList(Connection conn) {
		...
		try {
			...
			while (rs.next()) {
				Board b = new Board();
				b.setBoardNo(rs.getInt("b.board_no"));
				b.setBoardTitle(rs.getString("b.board_title"));
				b.setBoardContent(rs.getString("b.board_content"));
				b.setMemberName(rs.getString("m.member_name"));
				b.setRegDate(rs.getTimestamp("b.reg_date").toLocalDateTime());
				b.setModDate(rs.getTimestamp("b.mod_date").toLocalDateTime());
				resultList.add(b);
			}
		} catch (Exception e) {
			e.printStackTrace();
