		...
		<div id="section_wrap">
			...
			<div class="update_member_form">
				<form action='/memberUpdateEnd' name="update_member_form" method="post">
					...
					<input type="text" name="member_id" value="<%=m.getMemberId()%>" readonly disabled>
					<br>
					<input type="password" name="member_pw" placeholder="수정 비밀번호">
					<br>
					<input type="password" name="member_pw_check" placeholder="수정 비밀번호 확인">
					<br>
					<input type="text" name="member_name" value="<%=m.getMemberName()%>" placeholder="닉네임">
					<br>
	...
	<script>
		const updateMemberForm = function() {
			...
			if (!form.member_id.value) {
				...
				//form.submit();
				// 1. 경로 : /memberUpdateEnd -> MemberUpdateEndServlet(암호화 필터)
				// 2. 타입 : post
				// 3. 응답 방식 : JSON
